...
**Tech Stack:** PostgreSQL 18, pgschema 1.10.0 (declarative schema), Scythe 0.7.0 (`python-psycopg3` codegen → `lib/db`), squawk (plan-safety lint), psycopg3 async, uv workspace.
...
- **Tools assumed installed locally:** `pgschema` 1.10.0, `scythe` 0.7.0, `squawk`, `psql`, `docker`, `uv`. (CI installs them itself — Task 5.)
...
Replace the entire file with the version below. Scythe reads `sql/schema.sql` statically, so the Postgres preflight, `dbmate up`, `dbmate dump`, and the `\`-strip step are all removed. The import-patch (gotcha #2) and the package-marker `touch` are retained.
...
# 1. Generate Python from sql/schema.sql + sql/queries/*.sql.
scythe generate

# 2. Patch the two imports Scythe omits (it emits uuid.UUID and dict[str, Any]
#    but no `import uuid` / `from typing import Any`).
python3 - "$GEN_DIR" <<'PY'
import pathlib
import sys
...
for f in gen.glob("*.py"):
    if f.name == "__init__.py":  # never patch the package marker
        continue
    text = f.read_text()
    additions = []
    if "uuid." in text and "import uuid" not in text:
        additions.append("import uuid  # noqa: F401\n")
    if "Any" in text and "from typing import Any" not in text:
        additions.append("from typing import Any  # noqa: F401\n")
    if not additions:  # idempotent: nothing left to add
        continue
    lines = text.splitlines(keepends=True)
    lines.insert(1, "".join(additions))  # after the module docstring on line 0
...
- [ ] **Step 1: Write `scripts/db-apply.sh`**

Create the file below. It derives `PG*` connection env vars from `DATABASE_URL` (keeping a single DSN source), prints the plan, then applies. Extra args pass through to `pgschema apply` (e.g. `--auto-approve` in CI).
...
# DATABASE_URL so the repo keeps one DSN source. Extra args pass through to
# `pgschema apply` (e.g. --auto-approve, --lock-timeout 30s).
set -euo pipefail
...
print(f'export PGHOST={p.hostname or "localhost"}')
print(f'export PGPORT={p.port or 5432}')
print(f'export PGUSER={p.username or ""}')
print(f'export PGPASSWORD={p.password or ""}')
print(f'export PGDATABASE={(p.path or "/").lstrip("/")}')
print(f'export PGSSLMODE={q.get("sslmode", "prefer")}')
PY
)"
...
Update `## Codegen pipeline — Scythe (pinned 0.7.0)` (around line 151):
